...
def pdf_file(filename, content, password):
    pdf = HTML(string=content).write_pdf()

    pdf_filename = f"{filename}.pdf"

    local_pdf_path = os.path.join(settings.MEDIA_ROOT, pdf_filename)
    with open(local_pdf_path, "wb") as local_pdf_file:
        local_pdf_file.write(pdf)
    ...
    local_encrypted_filepath = os.path.join(settings.MEDIA_ROOT, encrypted_filename)
    with open(local_encrypted_filepath, "wb") as f:
        writer.write(f)

    os.remove(local_pdf_path)

    return upload_file(local_encrypted_filepath, encrypted_filename)
